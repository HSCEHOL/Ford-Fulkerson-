...
**에드몬드 카프(Edmonds-Karp)** 와 **포드 풀커슨(Ford-Fulkerson)** 라는 알고리즘을 주로 사용한다. 이 보고서는 
**포드 풀커슨(Ford-Fulkerson)** 알고리즘을 다루기 위한 것이므로 **포드 풀커슨(Ford-Fulkerson)** 알고리즘을 중심으로 내용을 전개 해 갈 것이고 , 이것을 설명하기 위해선 Network Flow가 무엇인지에 대해 다뤄야 할 필요가 있다. <br><br>

후에 설명하겠지만 , 결국에는 Network Flow 라는 것 자체가 **포드 풀커슨 알고리즘**이고 , **에드몬드 카프 알고리즘** 이다.  
<br><br><br>
...
실제로 계산한 결과와 같은 값이 나온다는 것을 알 수 있다. 하지만 이 알고리즘에는 단점이 없을까?
여기저기서 정보를 찾아보다보니 사람들은 포드 풀커슨 알고리즘보다 에드몬드 카프 알고리즘을 사용하는것이 더 효율적이라고 말한다.
두 알고리즘이 비교되는 이유는 무엇일까? 포드 풀커슨 알고리즘의 단점을 확인해보면서 이러한 사항을 알아보자.
...
위에 보면 알 수 있듯이  DFS(깊이 우선 탐색)을 탐색기법을 사용하는 **포드 풀커슨 알고리즘**은
